...
function FriendList({ data, sendRequest }) {
    return (
        <>
            {
                data?.length > 0
                                ...
                                <div className="col-md-6">
                                    <div className="border d-flex gap-3 p-3 mb-2">
                                        ...
                                        <div>
                                            ...
                                            <button className="btn btn-success px-2 py-1" onClick={()=>sendRequest(v?.id)}>
...
export default function Friends() {
    ...
    async function findFriends() {
        setLoading(true)
        const response = await ApiClass.getRequest(`friends/find?page=${page}&per_page=${per_page}&name=${name}`, true);
        setLoading(false)
    ...
    const sendRequest = async(id) => {
        ...
        const response = await ApiClass.putRequest(`friends/add/${parseInt(id)}`, true);
        if(response.data.status_code == 0){
            return toast.error(response?.data?.message)
        }
        if(response.data.status_code == 1){
                ...
                <div className="row justify-content-center">
                    {
                        loading
                            ...
                            :
                            <FriendList data={suggestions} sendRequest={sendRequest}/>
                    }
                </div>
