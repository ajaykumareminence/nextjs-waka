...
function FriendList({ data, cancelRequest }) {
    return (
        <>
            {
                data?.length > 0
...
export default function Friends() {
    ...
    async function findFriends() {
        setLoading(true)
        const response = await ApiClass.getRequest(`friends/sent_requests?page=${page}&per_page=${per_page}&name=${name}`, true);
        setLoading(false)
                ...
                <div className="row justify-content-center">
                    {
                        loading
                            ...
                            :
                            <FriendList data={suggestions} cancelRequest={cancelRequest}/>
                    }
                </div>
